...
def preprocess_crime_data(SPD_file_name):
    ...
    """
    df = pd.read_csv(SPD_file_name,
                     usecols=["Report Number", "Offense Start DateTime",
                              "Report DateTime", "Offense Parent Group",
                              "Offense Code", "MCPP", "Longitude",
                              "Latitude"]).dropna()
...
def add_zip_column(df, geolocator):
    df['zipcode'] = df.head().apply(lambda x: get_zip_code(x, geolocator),
                                    axis=1)
    return df
...
def add_income(income_file_name, crime_data):
    ...
    merged_data = crime_data.merge(income_data,
...
def main():
    cleaned_crime_data = preprocess_crime_data(
        'crime_analysis/SPD_Crime_Data.csv'
        )
    geolocator = geopy.Nominatim(user_agent='myusername')
    zip_crime_data = add_zip_column(cleaned_crime_data, geolocator)
    income_crime_data = add_income('crime_analysis/median_income.csv',
                                   zip_crime_data)
